...
flags.DEFINE_multi_integer("horizon",
                     default=[1,2,4,8],
...
flags.DEFINE_float("lambda_init",
                     default=0.95,
                     help="initial eigenvalue of latent state transition matrix")
flags.DEFINE_integer("train_steps",
                     default=100,
                     help="number of GD optimization steps")
flags.DEFINE_string('output_dir',
                    default=os.path.join(Path(os.path.dirname(os.path.abspath(os.getcwd()))), "dfm_medium_data"),
...
d_data = data[['D' + str(i) for i in range(1, 13+1)]].dropna()
m_data = data[['M' + str(i) for i in range(1, 20+1)]].dropna()
GDP_data = data[['Q1']].dropna()

# Interpolate non-filled daily data
d_data_interp = pd.concat(
    [daily_begginingInterp(d_data[['D' + str(i)]], length=FLAGS.days_month, endOfMonth=False)
        for i in range(1, 13+1)],
    join = 'outer', axis = 1
)
d_data_interp.columns = ['D' + str(i) for i in range(1, 13+1)]

# Filled data
m_data_prefill = preproc_data[['M' + str(i) for i in range(1, 20+1)]].loc[
...
d_data_fill_interp = pd.concat(
    [daily_begginingInterp(d_data[['D' + str(i)]], length=FLAGS.days_month, endOfMonth=True)
        for i in range(1, 13+1)],
    join = 'outer', axis = 1
)
d_data_fill_interp.columns = ['D' + str(i) for i in range(1, 13+1)]
...
linear_gaussian_model = QMWDDFM(non_mask_ar = FLAGS.non_mask_ar,
  ...
  model = FLAGS.model, lambda_init = FLAGS.lambda_init)
...
for horizon in FLAGS.horizon:
  print(horizon)
  #kalman_filter_predictions_train_ = linear_gaussian_model.predict(y_test = data_train, predict_dates = train_predict_dates,
  #                                                                horizon = horizon)
  kalman_filter_predictions_train = linear_gaussian_model.predict_graph(y_test = data_train, predict_dates = train_predict_dates,
                                                                  horizon = horizon)
...
for s in np.arange(0, data_test.shape[0]+update_freq, update_freq):
  ...
  if s % (update_freq) == 0:
    ...
    for horizon in FLAGS.horizon:
      print(horizon)
      #do not compute the latent path from the beginning but only use some long window
      path_lag = 1000
      kalman_filter_predictions_test = linear_gaussian_model.predict_graph(y_test = data_s.iloc[-path_lag:],
                                                                      predict_dates = data_s.iloc[-path_lag:].index,
                                                                      horizon = horizon)
...
for horizon in FLAGS.horizon:
  ...
  q_predictions_test = pd.concat(predictions_test_horizon[horizon], 0).droplevel(0).loc[q_dates_test][[i for i in range(q_dims)]].rename(columns = {i: sym for i, sym in zip(range(q_dims), q_symbols)})
  m_predictions_test = pd.concat(predictions_test_horizon[horizon], 0).droplevel(0).loc[m_dates_test][[i for i in range(q_dims, q_dims + m_dims)]].rename(columns = {i: sym for i, sym in zip(range(q_dims, q_dims + m_dims), m_symbols)})
  for m_symbol in m_symbols:
    fig, axes = plt.subplots(nrows = 1, ncols = 1, figsize = (6, 6))
    df_m_test = pd.concat([pd.DataFrame(m_predictions_test[m_symbol]).rename(columns = lambda s: s + '_test_pred'),
                            pd.DataFrame(m_data_test[m_symbol]).rename(columns = lambda s: s + '_test')], 1)
  ...
  for q_symbol in q_symbols:
    fig, axes = plt.subplots(nrows = 1, ncols = 1, figsize = (6, 6))
    df_q_test = pd.concat([pd.DataFrame(q_predictions_test[q_symbol]).rename(columns = lambda s: s + '_test_pred'),
                            pd.DataFrame(q_data_test[q_symbol]).rename(columns = lambda s: s + '_test')], 1)

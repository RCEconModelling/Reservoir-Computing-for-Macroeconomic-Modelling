...
#
# Original Python code:  Michael Gong
#   https://michael-gong.com/blogs/model-confidence-set/
# Reference MATLAB code:  MFE Toolbox, Kevin Sheppard
...
def bootstrap_sample(data, B, w, seed=None):
    '''
    ...
    indices[0, :] = np.ceil(t * rng.random((1, B)))
    select = np.asfortranarray(rng.random((B, t)).T < p)
    vals = np.ceil(rng.random((1, np.sum(np.sum(select)))) * t).astype(int)
...
"""
def bootstrap_sample(data, B=1, w=1, seed=None):
    '''
    Bootstrap the input data
    data: input numpy data array
    B: boostrap size
    ...
    '''
    # See moving block bootstrap of Kunsch (1989), AoS
    N = len(data)
    n = N - w + 1
...
def MCS(losses, alpha, B, w, algorithm, seed=None):
    '''Main function of the MCS'''
    t, M0 = losses.shape
    bsdata = bootstrap_sample(np.arange(t), B, w, seed)
...
class ModelConfidenceSet(object):
    def __init__(self, data, alpha, B, w, algorithm="SQ", seed=None, names=None):
        """
        ...
        if not seed is None:
            if not isinstance(seed, int):
                raise TypeError(f"Seed must be either None or an integer, found {type(seed)}")
    ...
    def run(self):
        included, excluded, pvals = MCS(
            self.data, self.alpha, self.B, self.w, self.algorithm, self.seed
        )

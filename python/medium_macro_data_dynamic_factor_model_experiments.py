...
flags.DEFINE_string('output_dir',
                    default=os.path.join(Path(os.path.dirname(os.path.abspath(os.getcwd()))),
                                         "dfm_results_medium"),
...
def normalize_data(train):
  ...
  m_train_d = train[['D' + str(i) for i in range(1, 13+1)]].mean()
  s_train_d = train[['D' + str(i) for i in range(1, 13+1)]].std()
  m_train_m = train[['M' + str(i) for i in range(1, 20+1)]].loc[
    train.index.day == FLAGS.days_month].mean()
  s_train_m = train[['M' + str(i) for i in range(1, 20+1)]].loc[
...
linear_gaussian_model.fit(y_train = data_train_normalised, train_dates = data_train.index, learning_rate=FLAGS.learning_rate,
...
observation_predictions_train = [m_train.to_numpy() + s_train.to_numpy() * p  for p in normalised_observation_predictions_train]
